...
import type { ConsolidationOutput } from "../../src/core/consolidation.ts";
import { ConsolidationPipeline } from "../../src/core/consolidation.ts";
...
function createMockLLM(consolidationResponse?: ConsolidationOutput): LLMPort {
	return {
...
describe("ConsolidationPipeline — reinforce", () => {
	...
	test("adds sourceEpisodicId to existing fact", async () => {
		...
		// New episode that reinforces the fact
		const episode = makeEpisode();
		await storage.saveEpisode(userId, episode);
...
describe("ConsolidationPipeline — update", () => {
	...
	test("invalidates old fact and creates new one", async () => {
		...
		const facts = await storage.getFacts(userId);
		// Old fact invalidated, new fact created — only 1 valid fact
		expect(facts).toHaveLength(1);
...
describe("ConsolidationPipeline — invalidate", () => {
	...
	test("skips invalidate when existingFactId is missing", async () => {
		...
		// Schema validation should catch missing existingFactId for invalidate action,
		// but if it slips through, the pipeline should handle it gracefully
...
describe("ConsolidationPipeline — multiple episodes", () => {
	...
	test("later episodes see facts created by earlier episodes", async () => {
		...
		const llm = createDynamicMockLLM((messages) => {
			callCount++;
			if (callCount === 1) {
				// First episode: create a new fact
				return {
					facts: [
						{
							action: "new" as const,
			...
			// Second episode: should see the fact from first episode in the prompt
